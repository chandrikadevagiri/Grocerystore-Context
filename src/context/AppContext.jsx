...
export const AppContextProvider=({children})=>{
    ...
    const addToCart=(itemId)=>{
        let cartData=structuredClone(cartItems);//copy the data
        if(cartData[itemId]){
            cartData[itemId]+=1;
        }else{
            cartData[itemId]=1;
...
useEffect(()=>{
    ...
    const value={navigate,user,setUser,isSeller,setIsSeller,showUserLogin,setShowUserLogin,products,currency,
        cartItems,addToCart,updateCartItem,removeCartItem,searchQuery,setSearchQuery,getCartCount,getCartAmount
    }
