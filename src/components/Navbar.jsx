...
import carticon from '../assets/nav-cart-icon.png';
...
const Navbar = () => {
    ...
    const {user,setUser,setShowUserLogin,navigate,searchQuery,setSearchQuery,cartItems,addToCart,updateCartItem,removeCartItem,getCartCount}=useAppContext();
           ...
           <NavLink to='/'>
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                <div className="relative cursor-pointer">
                    <img src={carticon} alt='carticon' className='w-6 opacity-80'/>
                    <button  onClick={()=>{navigate('/cart');scrollTo(0,0)}}className="absolute -top-2 -right-3 text-xs text-white bg-primary w-[18px] h-[18px] rounded-full">{getCartCount()}</button>
                </div>
            {!user ?(
            ...
            (
                <div className='relative group'>
                    ...
                    <ul className='hidden group-hover:block absolute top-10 right-0 bg-white shadowborder border-gray-200 py-2.5 w-30 rounded-md text-sm z-40'>
                        ...
                        <li onClick={()=>navigate('/my-orders')} className='p-1.5 p1-3 hover:bg-primary/10 cursor-pointer'>Myorders</li>
